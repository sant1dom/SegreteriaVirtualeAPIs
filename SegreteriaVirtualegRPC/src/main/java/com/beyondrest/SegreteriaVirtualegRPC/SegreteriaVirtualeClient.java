...
public class SegreteriaVirtualeClient {
    ...
    public List<Messaggio> getMessaggi(String idInsegnamento){
        ID request = ID.newBuilder().setId(Integer.parseInt(idInsegnamento)).build();
        BachecaResponse messaggi = blockingStub.getMessaggi(request);
        logger.info("Messaggi: "+ messaggi.getBachecaList().stream().map(messaggio -> messaggio.getTitolo() + " " + messaggio.getTesto()).reduce((a, b) -> a + ", " + b).orElse(""));
        return messaggi.getBachecaList();
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            var messaggio = client.getMessaggio(insegnamento.getId(), messaggi.get(0).getId());
